...
## 2026-07-03 - A5-7 召回冷却与 breath 优化

### 1. 新增 recall cooldown

新增 `api/recall_cooldown.py`。

用于记录最近浮现过的 bucket，并在短时间内避免重复召回。
...
### 2. 自动 Scene Memory 不再 touch

`build_scene_memories()` 现在使用 `touch=False`。

这意味着自动注入背景记忆不会再增加 `use_count`，避免记忆因自动浮现而被越推越高。

### 3. memory_breath 模式扩展
...
同时也接入 recall cooldown。

### 4. Dashboard / Pulse

Dashboard Gateway 和 Memory Seeds 面板会显示：

- cooldown skipped
- cooldown active
- active bucket 剩余冷却秒数

Pulse 增加 `recall_cooldown` 状态。

### 5. 验证

已使用项目虚拟环境验证：

- `api/recall_cooldown.py`、`api/gateway.py`、`api/memory_tools.py`、`api/pulse.py`、`main.py` 语法检查通过。
- `memory_breath` 的 `natural` 模式可解析并执行。
- cooldown status 可正常返回。
