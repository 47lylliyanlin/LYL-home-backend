...
???????????????????????????????? `memory/archive/cleanup_20260630_174322`????????? `backups/memory_before_cleanup_20260630_174322`?
...
?????? Word Map Lite ? ChromaDB??? Scene Memory ???????????

### Scene Memory ????

`GET /api/gateway/last-context` ?? `scene_explanations`??????? prompt???? Dashboard ???????????

| ?? | ?? |
|------|------|
| keyword_score | ????????????domain?tags?????????? |
| vector_score | ChromaDB ?????? |
| base_score | ????????? type?pinned?importance?activation ??? |
| final_score | ??????? |
| importance | ????? |
| use_count | ???????? |
| pinned | ???? |
...
| ?? | ??? | ?? |
|------|--------|------|
| MEMORY_DIRECT_VECTOR_THRESHOLD | 8.0 | ??????? Scene Memory ??????????????? |
...
## 2026年7月2日补充：Internal Tool Loop MVP

`POST /api/chat` 现在支持 Gateway 内部工具循环的第一版能力。

当前工具：

| 工具 | 作用 | 是否写记忆 |
|------|------|------------|
| memory_breath | 由模型主动请求搜索 active long-term memory，返回少量 bucket 摘要和 id | 否 |

相关配置：

| 变量 | 默认值 | 说明 |
|------|--------|------|
| MEMORY_INTERNAL_TOOL_LOOP_ENABLED | true | 是否允许模型在文本聊天中请求 `memory_breath` |

调试字段：

| 字段 | 说明 |
|------|------|
| tool_loop_enabled | 当前是否启用 internal tool loop |
| tool_loop_requested | 本轮模型是否请求了工具 |
| tool_loop_request | 模型请求的工具和 query |
| tool_loop_result | 工具返回的记忆摘要，不包含 Darkroom 正文 |

用户不会看到中间工具 JSON，只会看到最终回复。

### Gateway 收缩配置

| 变量 | 默认值 | 说明 |
|------|--------|------|
| MEMORY_AUTO_SCENE_ENABLED | true | 是否保留 Gateway 自动 Scene Memory。过渡期为 true，后续可切到 false，让 AI 更多通过 tool loop 主动查记忆。 |

### Internal Tool Loop 新增工具

| 工具 | 作用 | 是否写记忆 |
|------|------|------------|
| memory_read_bucket | 根据 bucket_id 读取一条 active memory 的详情 | 否 |

`memory_read_bucket` 不读取 archive，不读取 Darkroom 正文，不修改 use_count。用户不会看到工具 JSON，只会看到最终回复。

### Internal Tool Loop：memory_trace

| 工具 | 作用 | 是否写记忆 |
|------|------|------------|
| memory_trace | 根据 bucket id 或 moment id 追踪相关 moments / edges | 否 |
...
`memory_trace` 只读取 graph 上下文，返回数量有限制，不读取 Darkroom 正文，不写入记忆。

---

## 2026-07-03 补充：memory_hold_candidate

`POST /api/chat` 的 Internal Tool Loop 新增 `memory_hold_candidate`。

这个工具用于让模型在聊天中主动提出“这条内容可能值得记住”，但它不会直接写入正式长期记忆，也不会修改 permanent bucket。

### 工具定位

| 工具 | 作用 | 是否直接写正式记忆 |
|------|------|------------------|
| memory_hold_candidate | 创建一条待确认的记忆候选 | 否 |
...
- 用户明确说“记住这个”。
- 发生稳定事实、承诺、项目状态或关系里程碑变化。
- 当前内容明显适合后续长期接续。

不应该用于：

- 刚刚 / 上一句 / 当前 session 内就能回答的问题。
- 对用户性格、身份、偏好的猜测。
- 普通闲聊语气、动作描写、临时情绪。
...
Dashboard 已新增 `Memory Candidate Queue`，用于查看模型提出的候选记忆。
